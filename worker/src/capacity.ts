...
export class CapacityLedger {
  ...
  benchmark(now = Date.now()) {
    return this.storage.transactionSync(() => {
      ...
      if (used >= 300) return { status: 429, body: { error: 'Daily evaluation request allowance reached.', code: 'benchmark_quota', retry_after_seconds: Math.ceil(((day + 1) * 86400000 - now) / 1000) } };
