...
subprocess.run(['agg','--font-family','Menlo','--font-size','20','--line-height','1.3',
    ...
    str(OUT/'edited.cast'),str(OUT/'terminal.gif')],check=True,stderr=subprocess.DEVNULL)
...
filters = [f'fps=24,pad={width}:{height}:64:120:color=0xf3f5ef',
    ...
    f'drawtext=fontfile={mono}:text=REAL UBUNTU SESSION / WAITS SHORTENED / NO SIGN-IN RECORDED:fontsize=13:fontcolor=0x536055:x=40:y=h-36']
...
subprocess.run(['ffmpeg','-y','-v','warning','-i',str(OUT/'terminal.gif'),'-filter_script:v',str(OUT/'filter.txt'),
    ...
    str(MEDIA/'bailout-demo-v1.mp4')],check=True)
subprocess.run(['ffmpeg','-y','-v','warning','-i',str(MEDIA/'bailout-demo-v1.mp4'),
    '-vf','fps=12,scale=960:-1:flags=lanczos,split[a][b];[a]palettegen=stats_mode=diff[p];[b][p]paletteuse=dither=bayer:bayer_scale=3',
    '-loop','0',str(MEDIA/'bailout-demo-v1.gif')],check=True)
# A real recorded frame showing the initial ask, before the screen scrolls.
poster_time = min(m['time'] for m in markers if m['title']=='Set up your tools')+.2
subprocess.run(['ffmpeg','-y','-v','error','-ss',str(poster_time),'-i',str(MEDIA/'bailout-demo-v1.mp4'),
    '-frames:v','1',str(MEDIA/'bailout-demo-v1.png')],check=True)
...
shutil.copyfile(OUT/'session.cast', MEDIA/'bailout-demo-v1.cast')
print(json.dumps(metadata,indent=2))
for path in sorted(MEDIA.glob('bailout-demo-v1.*')):
